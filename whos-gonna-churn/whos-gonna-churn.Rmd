...
```{r}
df <- read.csv('BankChurners.csv',stringsAsFactors = T)
...
There are a couple of valid ways to handle with an unbalanced dataset, some of those involve upsampling or undersampling datasets. A correct way (often not considered) is to use scale_pos_weight to easily handle with unbalanced target class in binary classification.

scale_pos_weight is defined as the ratio between: (negative cases)/(positive cases). In our case the ratio will be of 5.22 (\# of existing customers/ \# of attrited customers).
